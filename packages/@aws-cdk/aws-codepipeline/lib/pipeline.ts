...
export interface StageInsertionProps {
  /**
...
export class Pipeline extends cdk.Construct implements cpapi.IPipeline {
  ...
  private readonly stages = new Array<cpapi.IStage>();
   ...
   * @param props optional Stage insertion properties
   */
  public addStage(stage: cpapi.IStage, props: StageInsertionProps = {}): Pipeline {
    // check for duplicate Stages and names
    if (this.stages.find(s => s.stageName === stage.stageName)) {
      throw new Error(`Stage with duplicate name '${stage.stageName}' added to the Pipeline`);
    }

    (stage as any)._attachStageToPipeline(this);
  ...
  private findStageIndex(targetStage: cpapi.IStage) {
    return this.stages.findIndex((stage: cpapi.IStage) => stage === targetStage);
  }
