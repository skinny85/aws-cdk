...
function artifactAttribute(artifact: Artifact, attributeName: string) {
  return new CloudFormationToken(() => ({ 'Fn::GetArtifactAtt': [artifact.artifactName, attributeName] })).toString();
}

function artifactGetParam(artifact: Artifact, jsonFile: string, keyName: string) {
  return new CloudFormationToken(() => ({ 'Fn::GetParam': [artifact.artifactName, jsonFile, keyName] })).toString();
}
